...
public class NewsController {
    ...
    public String addNews(@Valid @ModelAttribute(value = "news") News news, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            return HTMLConstant.NEWS_PAGE_ADD;
        }
        newsService.addOrUpdateNews(news);
        return "redirect:" + MappingConstant.HOME;
    }

    @RequestMapping(value = MappingConstant.ADD_NEWS, method = RequestMethod.POST, params = "edit")
    public String editNews(@Valid @ModelAttribute(value = "news") News news, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
